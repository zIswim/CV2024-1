...
class Panorama(QMainWindow):
    def __init__(self):
        ...
        self.cutButton.setGeometry(310, 25, 100, 30)
    ...
    def stitchFunction(self):
        ...
        if status == cv.STITCHER_OK:
            ...
            cv.setMouseCallback('Image stitched panorama', self.drawing)
        else:
    ...
    def drawing(self, event, x, y, flags, param):
        ...
        if event==cv.EVENT_LBUTTONDOWN:
            ...
            cv.rectangle(self.img_stitched, (ix, iy), (x, y), (0, 0, 255), 2)
            ix2, iy2 = x, y
        cv.imshow('Image stitched panorama', self.img_stitched)

    def cutFunction(self):
        self.img_show = self.img_stitched[iy:iy2, ix:ix2]
        cv.imshow('Cutting', self.img_show)

    def saveFunction(self):
        fname = QFileDialog.getSaveFileName(self, '파일 저장', './')
        cv.imwrite(fname[0], self.img_show)
